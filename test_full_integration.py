...
def test_full_flow():
    print("Testing full flow: conversation creation, message creation, and task addition...")
    ...
    try:
        ...
        with Session(engine) as db:
            ...
            conversation = Conversation(
                **conversation_data.model_dump(),
                user_id=25  # Use the user ID from our test
            )
            db.add(conversation)
            db.commit()
            ...
            ).order_by(Message.created_at.asc())  # Changed from timestamp to created_at
        ...
        print("The chatbot should now be able to add tasks to the database!")
